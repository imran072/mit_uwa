...
def cleanData(lines):
    ...
    for line in lines[1:]:
        try:
            ...
            # Skip invalid rows
            if not region or not country or population <= 0 or land_area <= 0 or country_counts[country] >= 2:
                continue
...
def getRegionCountryStats(region_data):
    ...
    for region, countries in region_data.items():
        ...
        for country, data in countries.items():
            ...
            if population > 0:
                percentage_population = round((population / sum(data[0] for data in countries.values())) * 100, 4)
            density = round(population / land_area, 4)
            country_list.append([country, population, net_change, percentage_population, density])

        # Sort country_list based on population, density, and alphabetical order
        country_list.sort(key=lambda x: (x[1], -x[4], x[0]), reverse=True)

        # Add additional information to each country's data
        rank = 1
        for country_data in country_list:
            country, population, net_change, percentage_population, density = country_data
            country_data.extend([rank])
            rank += 1
